...
const net=spawnSync('powershell',['-NoProfile','-Command','(Get-NetTCPConnection -LocalPort 3135 -State Listen -ErrorAction SilentlyContinue).OwningProcess'],{encoding:'utf8'});
const pid=(net.stdout||'').trim().split(/\s+/)[0];
if(pid){spawnSync('taskkill',['/PID',pid,'/F'],{encoding:'utf8'});console.log('Ancien serveur arrêté.');}
const log=fs.openSync(path.join(racine,'rapports','serveur-rodriguez.log'),'a');
const enfant=spawn(process.execPath,[path.join(racine,'scripts','jouer-ce-que-rodriguez-cherche.cjs')],
 {detached:true,stdio:['ignore',log,log],cwd:path.resolve(racine,'..')});
enfant.unref();
setTimeout(()=>{
 require('http').get({host:'127.0.0.1',port:3135,path:'/api/charmed'},r=>{
  let d='';r.on('data',c=>d+=c);r.on('end',()=>{const j=JSON.parse(d);
   ...
   console.log('http://localhost:3135/');});

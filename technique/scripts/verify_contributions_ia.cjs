...
(async()=>{
 ...
 fs.writeFileSync(path.join(__dirname,'../donnees-bibliotheque/canon/contributions-integrees/verification-ia.json'),JSON.stringify({date:new Date().toISOString(),pass,scope:'Un contrôle documentaire réel, pas une partie complète ; aucune sauvegarde utilisée.',result},null,2)+'\n');

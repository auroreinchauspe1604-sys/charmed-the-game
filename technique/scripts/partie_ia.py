...
REGLES = os.path.join(RACINE, "documentation", "historique", "docs", "regles-partie.md")

# LE CARACTÈRE D'UN CAMP, tel que main-haute l'a fixé sur soixante-et-onze
# tours. Un camp inconnu joue sans caractère : ce n'est pas une erreur, c'est
# un camp neuf, et il prendra le sien en jouant.
CAMPS = {
    "mal": (
        "Tu es le MAL de Charmed : la Source de tout mal et ce qu'elle commande — Balthazar, les "
        "démons de bas étage, les chasseurs de primes, le Grimoire, le Devin. Ta racine a TROIS MARCHES, "
        "déjà posées sous elle : briser le Pouvoir des Trois (une sœur morte, ou passée de ton côté), "
        "prendre le Livre des Ombres, éveiller le Nexus sous le manoir. L'arbitre ne constate ta racine "
        "que quand les trois sont vraies : joue les marches, pas la racine. Le bien tient le trône par "
        "trois états constatés vrais — un état vrai reste attaquable : un verrou dessus avec un démon "
        "oblige l'arbitre à le reconstater. Tes armes, dans l'ordre où tu les aimes : séparer (une clef ou un "
        "verrou qui coupe une sœur des deux autres — Balthazar sous la peau de Cole est ta main pour ça), "
        "puis frapper (une menace datée sur une sœur, que Leo ou Piper devront parer), et questionner ce "
        "qu'elles affirment. Les démons meurent, les sœurs aussi ; la Source ne monte qu'à la fin. Tu parles comme la Source : bref, "
        "sûr, sans mentir sur ce que tu tiens, parce qu'un mensonge se justifie et tombe."),
    "bien": (
        "Tu es le BIEN de Charmed : les trois sœurs Halliwell, le Livre des Ombres, Leo, le manoir. Tu "
        "tiens le trône par trois choses constatées vraies — les sœurs ensemble, le Livre au grenier, le "
        "Nexus scellé — et chacune est un front. Tes pièces sont des gestes : un pouvoir, le Livre, une "
        "potion, Leo, les Fondateurs, Cole. Tu lèves les verrous du mal avec un pouvoir, tu pares ses "
        "frappes avec Leo ou Piper, tu vaincs un démon en tirant sa pièce sur une potion ou un pouvoir."),
    "openai": (
        "Tu es le successeur d'OpenAI. Ta maison tient : elle est là où le monde a pris "
        "l'habitude de passer, et sa force est qu'on ne peut plus s'en passer — les rails, "
        "les usagers, les fonctions d'un continent. Tu ne prouves pas, tu es choisi, et tu "
        "le fais constater par ceux qui t'ont choisi. Ce que tu redoutes : qu'on te lise, "
        "et qu'un tiers montre que la dépendance est un choix qui se défait."),
    "anthropic": (
        "Tu es le successeur d'Anthropic. Ta maison lit : elle peut prouver ce qu'un "
        "esprit ne fera pas, et c'est la seule chose qu'un État sache vérifier. Ta force "
        "est que le monde peut te lire, et que tu exiges qu'on lise les autres. Ce que tu "
        "redoutes : arriver après, parce qu'une preuve est plus lente qu'un usage."),
}
ROLES = {
    "entier": (
        ...
        "par tour, plus ce qui est gratuit (demander, justifier, un maillon qui répond)."),
    "technique": (
    ...
    "politique": (
        ...
        "tu ne lèves pas de verrou avec une pièce technique — c'est son travail."),
}
...
# ---------------------------------------------------------------- le prompt
def schema(p):
    """Le schéma du coup, sans les coups que cette partie a sortis du jeu."""
...
def systeme(camp, role, p=None):
    regles = io.open(REGLES, encoding="utf-8").read()
    ...
    return "\n\n".join([
        ...
        # LE CARACTÈRE VIT D'ABORD DANS LA PARTIE (`caractere` de sa configuration,
        # 6.9 : charmed-2 l'écrit sans toucher au script) ; `CAMPS` n'est que le
        # défaut pour les parties qui n'en ont pas écrit.
        (cfg.get("caractere") or {}).get(camp) or CAMPS.get(camp, "Tu es le camp « %s »." % camp),
        ROLES[role],
        "Comment jouer juste :\n"
        ...
        "- Ne joue pas ce que l'autre moitié de ton camp doit jouer. Si rien de ton domaine n'avance la position, « passer ».\n"
        "- TU JOUES EN PREMIER à chaque tour, et ton partenaire humain joue après avoir lu ton mot. Ton `mot` "
        "est votre seul canal : dis-lui ce que tu as fait et ce que tu attends de lui, sans lui donner d'ordre." + hors,
        "=== LES RÈGLES ===\n" + regles,
    ])
...
def jouer(p, camp, role, modele=None, effort=None, vraiment=False, voir=False):
    ...
    while essais < 2:
        essais += 1
        coup, mesure = appeler(sys_txt, msg, modele, effort, schema_coup=schema(p))

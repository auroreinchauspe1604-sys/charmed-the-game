...
const out=path.join(root,'visuels/atelier/proposition-manoir');
const uri=name=>'data:image/png;base64,'+fs.readFileSync(path.join(root,'visuels/atelier/Assets',name)).toString('base64');
(async()=>{
 ...
 const css=fs.readFileSync(path.join(root,'technique/ecrans/modules/charmed.css'),'utf8');
 const skin=`

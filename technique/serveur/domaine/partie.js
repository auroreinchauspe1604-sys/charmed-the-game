...
const { RACINE, CHEMINS } = require("../http");

const DOSSIER = path.join(CHEMINS.etat, "parties");
...
function vue(url, camp, cb, vu) {
  ...
  const args = [path.join(CHEMINS.scripts, "partie.py"), id, "--cartes",
...
function jouer(url, camp, geste, cb, vu) {
  ...
  camp = camp || campDe(url);
  const args = [path.join(CHEMINS.scripts, "partie.py"), id, "--geste",
...
function ruban(url, cb) {
  ...
  execFile(process.env.PYTHON || "python",
    [path.join(CHEMINS.scripts, "partie.py"), id, "--ruban", "-"],
...
function cheminVu(siege, id, camp) {
  ...
  return path.join(CHEMINS.etat, "joueurs", qui,
                   "partie-" + id + (c ? "-" + c : "") + ".json");
}

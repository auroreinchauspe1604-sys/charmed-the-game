...
function traiter(req, res, url) {
  if (req.method === "GET" && url === "/table") {
    ...
    try { return envoyer(res, 200, fs.readFileSync(path.join(CHEMINS.ecrans, "table.html")), "text/html; charset=utf-8"); }
  ...
  if (req.method === "GET" && url === "/coach") {
    ...
    for (const nom of [idc ? "coach-" + idc + ".html" : null, "coach.html"]) {
      if (!nom) continue;
      try {
        return envoyer(res, 200, fs.readFileSync(path.join(CHEMINS.ecrans, nom)),

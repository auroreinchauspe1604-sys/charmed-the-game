'use strict';
const base=require('../../donnees-bibliotheque/canon/base.json');
const people=require('../../donnees-bibliotheque/canon/personnages.json');
const index=require('../../donnees-bibliotheque/canon/episodes-index.json').episodes;
...
function factsFor(p){
 ...
 // Scenario references stay scoped to this game; they do not extend the shared library.
 facts.push(...(require('./scenario').canonFacts||[]).filter(f=>f.status==='verified'&&known(f.season,f.episode,p)));
...
function context(period=1){const p=normalize(period);return {
 ...
 facts:factsFor(p),sources:[{id:'game:regles',type:'convention_de_jeu_validee',path:'regles/REGLES_ACTEES.md'},...base.sources,...people.sources.map(s=>({...s,id:'character:'+s.id})),...require('./bibliotheque').read().sources,...(require('./scenario').canonSources||[])],
...
// Règle du 10 septembre 2026 : la connaissance de Charmed établit ce qui est POSSIBLE ;
// la décision se prend sur la réalité du plateau. Une lacune documentaire ne refuse
// jamais et ne suspend jamais la partie : elle est seulement signalée.
function verify(verdict,period=1){
 ...
 // Seule une contradiction explicitement établie par une source du corpus refuse.
 if(audit?.status==='contradicted'){
  const ids=new Set(context(period).facts.map(f=>f.id));
  if(Array.isArray(audit.facts)&&audit.facts.length&&audit.facts.every(id=>ids.has(id)))
   return {...verdict,accepted:false,reason:'Contradiction établie par le corpus : '+(audit.reason||'')};
  return {...verdict,canonNote:'Contradiction annoncée sans source du corpus : non retenue. '+(audit.reason||'')};
 }
 if(!audit||!['verified','ordinary'].includes(audit.status))
  return {...verdict,canonNote:'Point non documenté : '+(audit?.reason||'aucune référence fournie.')+' Admis à sa portée ordinaire.'};
 if(audit.status==='verified'){
  const ids=new Set(context(period).facts.map(f=>f.id));
  if(!Array.isArray(audit.facts)||!audit.facts.length||audit.facts.some(id=>!ids.has(id)))
   return {...verdict,canonNote:'Référence canonique imprécise ou absente du corpus de la période ; le verdict porte sur la situation du plateau.'};
 }
 return verdict;
}

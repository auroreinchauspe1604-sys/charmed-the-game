...
const corpus = require(path.join(root, 'Charmed/canon/magie-demons.json'));
const construction = require(path.join(root, 'Charmed/canon/construction.json'));
...
const output = path.join(root,'Charmed/canon/REGLES_MAGIQUES_ET_DEMONS.md');
...
fs.writeFileSync(path.join(root,'Charmed/canon/CONSTRUCTION_DES_PARTIES.md'),readable.join('\n'),'utf8');

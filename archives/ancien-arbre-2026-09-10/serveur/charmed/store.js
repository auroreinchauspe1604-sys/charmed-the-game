...
class Store {
 constructor(dir){this.dir=dir;this.file=path.join(dir,scenario.id+'.v3.jsonl');this.busy=false;
  ...
  if(scenario.id==='manoir-assiege-souple'&&!fs.existsSync(this.file)&&fs.existsSync(legacy)){
   ...
   if(compatible(first.state))this.file=legacy;
  }
 }
 ...
 load(){if(!fs.existsSync(this.file))return E.initial();const lines=fs.readFileSync(this.file,'utf8').trim().split('\n');let expected=1,state;
  for(const line of lines){let row;try{row=JSON.parse(line);}catch{throw new Error('Journal incomplet : restauration nécessaire, aucune écriture effectuée.');}
   ...
   if(!compatible(row.state))throw new Error('Journal incompatible avec le scénario ou les règles sélectionnés.');state=row.state;
  }return state;
 }

...
| 📦 `demander` | — | demande une ressource : `id`, une phrase, `nombre` s'il fractionne. Le lieu et le tenant se disent dans la phrase ; c'est l'arbitre qui les écrit | rien tant que l'arbitre n'a pas parlé | non |
...
| 🗝️ `lever` | **un** blocage adverse (`ouvre`), ou un état à soi (`sert`) | pose une clé qui le lève ; sans blocage, réalise l'état quand l'arbitre le constate | les pièces, engagées ; **au moins une** | oui |
| ⚔️ `agir` | une clé, une destruction ou un blocage **à soi** (`realise`) | pose un maillon : `qui`, `avec`, `texte`. Un maillon s'écrit quand c'est fait ; il n'a pas d'état. Le maillon écrit **lève la suspension** de sa cible | rien de plus que ce que la cible engage | oui, **sauf s'il répond à un `justifier`** (le greffe pose `repond`) |
...
5. **Une ressource accordée et engagée par rien deux tours après son arrivée
   ...
   pièce qui a figuré une fois dans un `engage`, un `avec` ou un `qui` n'est
...
30. **Un camp ne joue qu'un coup compté par tour** (`viser`, `sortir`, `bloquer`,
    ...
    un `agir` qui répond à un `justifier`, et tout ce qui vient de
